...
class RegistrationForm(FlaskForm):
    ...
    def validate_email(self, email):
        ...
        if email: raise ValidationError('That email is taken. Please choose a different one.')
...
class RequestResetForm(FlaskForm):
    ...
    def validate_email(self, user):
        email = User.query.filter_by(email=email.data).first()
        if email == None: 
            raise ValidationError('There is no account under that email. You must register first.')
